...
<body>
    ...
    <div class="container py-5">
        ...
        <div id="anime-title" class="h3"></div>
        ...
        <div class="list-group d-hide mb-5" id="anime-episode-server"></div>

        <div class="h6 mb-3">Episodes</div>
        <div id="anime-episode-list" class="mb-3 row">
        </div>
    </div>
    ...
    <?php if ( !empty( $_GET['episode'] ) ) : ?>
        ...
        var base_url = '';
        var api_base_url = 'https://www9.gogoanime.io';
         ...
         * @returns {[]}
         */
        function anime_episode_list( source )
        {
            var $source = $( source );
            var epBtn = $source.find( '#episode_page a.active' );
            // var epBegin = parseInt( epBtn.attr( 'ep_start' ) );
            var epEnd = parseInt( epBtn.attr( 'ep_end' ) );
            var slug = $source.find( 'input#alias_anime' ).val();
            var epCurrent = parseInt( $source.find( 'input#default_ep' ).val() );
            var epTitle = $source.find( '.anime-info a' ).text() + ' ' + epCurrent;
            console.log( 'epTitle', epTitle );

            var list = [];
            var listHtml = [];
            for ( var i = 1; i <= epEnd; i++ ) {
                var uri = '/anime.php?episode=/' + slug + '-episode-' + i;
                list.push( uri );
                var btn = '<a href="' + uri + '" class="'+ ( epCurrent === i ? 'active' : '' ) +' btn btn-outline-secondary d-block mb-2">' + i + '</a>';
                listHtml.push( '<div class="col-3">'+ btn +'</div>' );
            }

            $( '#anime-episode-list' ).html( listHtml.join( '' ) );
            $( '#anime-title' ).html( epTitle );
        ...
        function anime_episode_server_service()
        {
            anime_get_source(url, function( source ) {
                var a = anime_episode_list( source );
